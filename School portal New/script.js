...
function validateInputs(studentData) {
    if (!studentData.session || !studentData.term || !studentData.className || !studentData.name || !studentData.department || !studentData.attendance || !studentData.performanceComment) {
        return false;
    }
...
function updateCSVData(studentData) {
    ...
    let csvData = localStorage.getItem(csvKey) || 'Session,Term,Class,Name,Department,Image Path,Attendance,Performance Comment,Subject,C.A Score,Exam Score,Total Score,Password\n';
    ...
    for (const subjectScore of studentData.subjectScores) {
        const row = `\n${studentData.session},${studentData.term},${studentData.className},${studentData.name},${studentData.department},${studentData.imagePath},${studentData.attendance},${studentData.performanceComment},${subjectScore.name},${subjectScore.ca},${subjectScore.exam},${subjectScore.ca + subjectScore.exam},${studentData.password}`;
        csvData += row;
    }
